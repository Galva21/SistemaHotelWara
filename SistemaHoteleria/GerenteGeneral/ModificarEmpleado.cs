...
namespace SistemaHoteleria.GerenteGeneral
{
    public partial class ModificarEmpleado : Form
    {
        ...
        public ModificarEmpleado(string id)
        {
            InitializeComponent();
            ...
            using (SistemaHotelWaraEntitiesV1 DB = new SistemaHotelWaraEntitiesV1())
            {
                try
                {
                    dgvEmpleados.DataSource = (from d in DB.Gerente
                                               where d.idEmpleado == idEmpleadoConectado
                                               select d).ToList();
                }
                catch (Exception)
                {
        ...
        {
            DialogResult result = MessageBox.Show("Esta seguro que desea realizar la modificacion del empleado?", "Mensaje de Confirmacion", MessageBoxButtons.YesNo);
