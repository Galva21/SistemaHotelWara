...
namespace SistemaHoteleria.RecepcionistaHotel
{
    ...
    {
        string idE;
        public NuevaReserva(string idEmpleado)
        {
            InitializeComponent();
            idE = idEmpleado;
            limpiarCampos();
        ...
        public void CargarHuespedes(string apellido)
        {
            using (SistemaHotelWaraEntitiesV1 DB = new SistemaHotelWaraEntitiesV1())
            {
                try
                {
                    ...
                    {
                        dgvHuespedes.DataSource = (from d in DB.Huespedes
                                                   where d.paterno.StartsWith(apellido) || d.materno.StartsWith(apellido) || d.documento.StartsWith(txtFiltroApellido.text)
                                                   select d).ToList();
                    }
        ...
        private void lblIdHabitacion_Click(object sender, EventArgs e)
        {

        }

        private void lblIdReserva_Click(object sender, EventArgs e)
        {
